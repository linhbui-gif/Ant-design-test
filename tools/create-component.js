...
const createComponentFile = (args, wrapperDirPath) => {
  const [compName] = args;
  const filePath = `${wrapperDirPath}/${compName}.jsx`;
...
const createIndexFile = (args, wrapperDirPath) => {
  const [compName] = args;
  const filePath = `${wrapperDirPath}/index.js`;
...
const createStorybook = (args, wrapperDirPath) => {
  ...
  const filePath = `${wrapperDirPath}/${compName}.stories.jsx`;

  const content = `import React from 'react';
import { Story } from '@storybook/react';

import ${compName} from '@/${dir}/${compName}';
