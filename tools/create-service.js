...
const COMMANDS = {
  ...
  FORMAT:
    'prettier --write src/services/**/**/*.js && prettier --write src/services/**/*.js && prettier --write src/redux/**/**/*.js && prettier --write src/redux/**/*.js',
};
...
const common = (args, mainPath) => {
  ...
  const rootPath = mainDirIsServices ? `${mainPath}/${service}/index.js` : mainPath;
  ...
  const fileName = `${kebabCaseActionName}.js`;
...
const createServiceFile = (args) => {
  ...
  const rootEndpoint = endpointSplitted[1];
  const pathArgs = endpointSplitted.filter((item) => item[0] === '{' && item[item.length - 1] === '}');
  const notHaveAnyPathArgs = arrayIsEmpty(pathArgs);
  const endpointJSSyntax = notHaveAnyPathArgs
    ? endpoint
    : `/${rootEndpoint}/${pathArgs
        .map(
          (arg) =>
            `$${arg.replace(
              removeFirstAndLastCharater(arg),
              `${API_MATERIALS.PATHS}?.${removeFirstAndLastCharater(arg)}`,
            )}`,
        )
        .join('/')}`;
...
const createSagaFile = (args) => {
  ...
  const importScope = `
    ...
    import { ${camelCaseActionName} } from '@/services/${service}';
  `;

  const functionScope = `
    export function* ${camelCaseActionName}Saga(action) {
      const { materials, successCallback, failedCallback } = action.payload;
      try {
        const response = yield call(${camelCaseActionName}, materials);
        const ${camelCaseActionName}Response = response;
...
const createReducerFile = (args) => {
  ...
  const functionScope = `
    export const ${camelCaseActionName}UpdateState = (state, action) => ({
      ...state,
